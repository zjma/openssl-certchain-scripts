...
openssl x509 -req -in %ReqFile% -extfile conf/rsa-subca.cnf -days 2000 -set_serial %RANDOM% -CA %ParentChainFile% -CAkey %ParentKeyFile% -out %CertFile% || EXIT /b 3
TYPE %CertFile% %ParentChainFile% > %ChainFile% || EXIT /b 5
EXIT /b 0

:USAGE_THEN_EXIT
ECHO Usage: gen-rsa-subca.bat ^<parent-name^> ^<user-name^> [sec-param=2048]
EXIT /b 1
