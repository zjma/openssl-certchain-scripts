...
openssl x509 -req -in %ReqFile% -extfile conf/ec-enduser.cnf -days 2000 -set_serial %RANDOM% -CA %ParentChainFile% -CAkey %ParentKeyFile% -out %CertFile% || EXIT /b 4
TYPE %CertFile% %ParentChainFile% > %ChainFile% || EXIT /b 5
EXIT /b 0

:USAGE_THEN_EXIT
ECHO Usage: gen-ec-enduser.bat ^<parent-name^> ^<user-name^> [curve-name=prime256v1]
EXIT /b 1
