...
function show_usage()
{
    printf "Usage: ./gen-ec-subca.sh <parent-name> <user-name> [curve-name=prime256v1]\n"
}
...
openssl x509 -req -in $ReqFile -extfile conf/ec-subca.cnf -days 2000 -set_serial $RANDOM -CA $ParentChainFile -CAkey $ParentKeyFile -out $CertFile
