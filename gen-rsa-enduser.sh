...
function show_usage()
{
    printf "Usage: ./gen-rsa-enduser.sh <parent-name> <user-name> [sec-param=2048]\n"
}
...
openssl x509 -req -in $ReqFile -extfile conf/rsa-enduser.cnf -days 2000 -set_serial $RANDOM -CA $ParentChainFile -CAkey $ParentKeyFile -out $CertFile
